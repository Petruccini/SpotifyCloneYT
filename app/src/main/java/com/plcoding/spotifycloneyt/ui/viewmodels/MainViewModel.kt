...
class MainViewModel @ViewModelInject constructor(
    ...
    fun skipToNextSong() {
        musicServiceConnection.transporControls.skipToNext()
    }

    fun skipToPreviousSong() {
        musicServiceConnection.transporControls.skipToPrevious()
    }

    fun seekTo(pos: Long) {
        musicServiceConnection.transporControls.seekTo(pos)
    }
    ...
    fun playOrToggleSong(mediaItem: Song, toggle: Boolean = false) {
        ...
        if (isPrepared && mediaItem.mediaId ==
            ...
            playbackState.value?.let { playbackState ->
                when {
                    playbackState.isPlaying -> if(toggle) musicServiceConnection.transporControls.pause()
                    playbackState.isPlayEnabled -> musicServiceConnection.transporControls.play()
                    else -> Unit
                }
            }
        } else {
            musicServiceConnection.transporControls.playFromMediaId(mediaItem.mediaId, null)
        }
    }
